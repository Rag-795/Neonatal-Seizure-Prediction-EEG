...
# Function to load and compute statistics from multiple non-seizure CSV files
def compute_channel_stats(non_seizure_files, channels):
    ...
    for file_path in non_seizure_files:
...
# Function to generate synthetic non-seizure EEG data for one sample
def generate_synthetic_data(stats, num_samples=7680, fs=256):
    ...
    for channel in CHANNELS:
        # For non-seizure, ECG EKG-REF is null in some cases
        if channel == "ECG EKG-REF" and np.random.random() < 0.5:
            data[channel] = [np.nan] * num_samples
            continue
        mean = stats[channel]['mean']
        std = stats[channel]['std']  # No amplitude scaling for non-seizure
...
if __name__ == "__main__":
    ...
    non_seizure_folder = "non_seizure_samples"
    output_folder = "synthetic_data"
    
    # Create output folder if it doesn't exist
    os.makedirs(output_folder, exist_ok=True)
    
    # Find all non-seizure files
    non_seizure_files = glob.glob(os.path.join(non_seizure_folder, "eeg*_non_seizure_sample.csv"))
    if not non_seizure_files:
        raise FileNotFoundError("No non-seizure sample files found in non_seizure_samples folder")
    
    # Load statistics from non-seizure files
    non_seizure_stats = compute_channel_stats(non_seizure_files, CHANNELS)

    # Generate 100 synthetic non-seizure samples
    for i in range(1, 101):  # Generate samples 1 to 100
        synthetic_non_seizure = generate_synthetic_data(non_seizure_stats, num_samples=7680)
        output_file = os.path.join(output_folder, f"synthetic_non_seizure_eeg_{i}.csv")
        synthetic_non_seizure.to_csv(output_file, index=False)
        print(f"Generated: {output_file}")

    # Generate and save synthetic annotations
    synthetic_annotations = generate_synthetic_annotations(num_samples=100)
    synthetic_annotations.to_csv(os.path.join(output_folder, "synthetic_non_seizure_annotations.csv"), index=False)

    print("\nSynthetic non-seizure data and annotations generated and saved in synthetic_data folder:")
    print("- synthetic_non_seizure_eeg_1.csv to synthetic_non_seizure_eeg_100.csv")
    print("- synthetic_non_seizure_annotations.csv")